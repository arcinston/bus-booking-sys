...
router.post('/admin/register', async (req, res) => {
  ...
  var exist = '';
  try {
    await AdminModel.findOne({ email: email }, (err: Error, val: any) => {
      if (err) {
        console.log(err);
      } else {
        exist = val;
    ...
    if (exist) {
      ...
      const html =
        '<p>Hi ' +
        admin.fname +
        ',<br><br> You have been registered as an admin to the public transport ticketing system.<br><br>To activate your account, please click the following link and sign in now.<br><br>Please use your <b>NIC Number</b> as your password to sign in.<br><br>' +
        configs.backendUrl +
        '/admin/reg/' +
